 ...
 */
var chapterTrainCtrl = m.controller("chapterTraincontroller",function ($rootScope,$scope) {

    /**
     * 章节练习初始化
     */
    $rootScope.ct_init = function () {
        $scope.prevPage = "上一题";
        $scope.nextPage = "下一题";
        ...
        $scope.chapterType = 1;
        $scope.questionSortConst = 1;
        $scope.resetCheck();
        $scope.getQuestionListOfQuestionBankByChapter();
    };
    /**
     * 获取章节题库分页列表
     */
    $scope.getQuestionListOfQuestionBankByChapter = function(){
        ...
        $.ajax({
            type:"POST",
            url:"/questionBank/getQuestionListOfQuestionBankByChapter",
            data:{"currentPage":this.currentPage,"pageSize":1,"chapter":$scope.chapterType},
    ...
    $scope.sortConstFunc = function ($event) {
        ...
        $scope.chapterType = $event.target.innerHTML;//第几章(后台获取)
